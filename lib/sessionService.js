...
async function login(token) {
    ...
    const { permissions, individualId } = user;
    const entitlementProfile = {
        individualId,
        permissions,
    };

    const sessionId = await driver.createSession(entitlementProfile);
    return sessionId;
}
/**
 ...
 * @returns {string[]}
 */
async function getSessionEntitlementsProfile(sessionId) {
